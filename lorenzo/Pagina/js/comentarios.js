  $(document).ready(function(){
    ...
    $("#addcomentario").click(function(){
      if(comentario.value != ""){
        ...
        $.get("../comentarios.php", {user:usuario, comentario:comentario.value, articulo:n}, function(data, status){
          $("#alerta").html("<div class='alert alert-success' role='alert' style='margin-top: 10px;'>Comentario publicado!</div>");
          document.getElementById('divcomentarios').innerHTML += "<ul class='media-list'><li class='media'><a href='#' class='pull-left'><img src='https://bootdey.com/img/Content/user_1.jpg' class='img-circle' style='margin-right: 10px;'></a><div class='media-body'><span class='text-muted pull-right'><small class='text-muted'>"+data+"</small></span><strong class='text-success'>@"+usuario+"</strong><p>"+comentario.value+"</p></div></li></ul><hr>";
          comentario.value = "";
        });
      }else{
